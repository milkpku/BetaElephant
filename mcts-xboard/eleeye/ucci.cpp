...
extern int COORD_MOVE(uint32_t dwMoveStr);
...
static bool ParsePos(UcciCommStruct &UcciComm, char *lp) {
	...
	if (StrScanSkip(lp, " moves ")) {
		*(lp - strlen(" moves ")) = '\0';
		UcciComm.nMoveNum = MMIN((int)(strlen(lp) + 1) / 5, MAX_MOVE_NUM); // 提示："moves"后面的每个着法都是1个空格和4个字符
...
UcciCommEnum IdleLine(UcciCommStruct &UcciComm, bool bDebug) {
	...
	if (false) {
		...
		UcciComm.nBanMoveNum = MMIN((int)(strlen(lp) + 1) / 5, MAX_MOVE_NUM);
		...
		return UCCI_COMM_NEWGAME;

		// 9. 无法识别的指令
	}
	else if (StrEqv(lp, "protover")){
		printf("feature myname = \"BetaElephant\"\nfeature memory = 1\nfeature smp = 1\nfeature setboard = 0 ping = 1 done = 0\nfeature variants = \"xiangqi\" done = 1\n");
		fflush(stdout);
		return UCCI_COMM_UNKNOWN;
	}
	else {
		if (strlen(lp) == 4){
			if ((lp[0] >= 'a') && (lp[0] <= 'i') && (lp[1] >= '0') && (lp[1] <= '9') && (lp[2] >= 'a') && (lp[2] <= 'i') && (lp[3] >= '0') && (lp[3] <= '9')){
				UcciComm.move = COORD_MOVE(*(uint32_t *)lp);
				return UCCI_COMM_MOVE;
			}
		}
		return UCCI_COMM_UNKNOWN;
	}
